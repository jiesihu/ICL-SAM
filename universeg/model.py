...
class UniverSeg(nn.Module):
    ...
    def forward(self, target_image, support_images, support_labels):
        ...
        for decoder_block in self.dec_blocks:
            ...
            # Add the attention map to the target map
    ...
    def forward_attention(self, target_image, support_images, support_labels,attention_map,alpha = 0.5):
        """
        attention_map: [1, 1, img size, img size]
        """
        attention_map = (attention_map-attention_map.mean())/attention_map.std()
        ...
        for decoder_block in self.dec_blocks:
            ...
            support = torch.cat([vmap(self.upsample, support), support_skip], dim=2)
            # Add the attention map to the target map
            attention_map_tmp = F.interpolate(attention_map.detach(), size=target.shape[-2:], mode="bilinear")
            attention_map_tmp = attention_map_tmp.view(1,1,1,target.shape[-2],target.shape[-1])
            

            norm_ori = support.norm().detach()            
            support = support*torch.exp(attention_map_tmp*alpha)
            support = support*norm_ori/support.norm().detach()
            target, support = decoder_block(target, support)
...
@validate_arguments
# Literal["v1"]
def universeg(version: str = "v1", pretrained: bool = False) -> nn.Module:
    weights = {
